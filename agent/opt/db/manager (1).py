...
class DatabaseManager:
    ...
    # ========================================================================
    # Mappe les statuts applicatifs vers les valeurs autorisees par le schema
    ...
    _EMAIL_STATUS_MAP = {
        ...
        # statuts internes -> rabattus sur une valeur valide
        "deduplicated": "PENDING",
    ...
    def insert_email_log(self, recipient, subject, status, alert_id=None,
                         error_message=None):
        try:
            db_status = self._EMAIL_STATUS_MAP.get(status, "FAILED")
            now = now_sqlite()
            with self.cursor() as cur:
                cur.execute("""
                ...
                """, (
                    str(uuid.uuid4()),
                    recipient[:200], subject[:500], db_status, alert_id,
                    error_message, now,
                    now if db_status == "SENT" else None,
                ))
                return cur.lastrowid
        except Exception as e:
